...
import { isIdentityVaultBackup, isIdentityVaultStatus, isPortableDid } from '../utils/identity-vault.js';
...
export class DcxIdentityVault implements IdentityVault<{ InitializeResult: string }> {
    ...
    public async restore({ backup, password }: { backup: IdentityVaultBackup; password: string; }): Promise<void> {
    ...
    async changePassword({ oldPassword, newPassword }: { oldPassword: string; newPassword: string; }): Promise<void> {
        throw new Error('Method not implemented.');
    }
    ...
    public async initialize({ password, recoveryPhrase, dwnEndpoints }: DcxIdentityVaultInitializeParams): Promise<string> {
