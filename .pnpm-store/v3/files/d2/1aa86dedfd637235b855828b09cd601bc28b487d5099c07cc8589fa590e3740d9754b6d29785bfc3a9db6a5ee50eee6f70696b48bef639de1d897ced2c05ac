...
import { VerifiableCredential, DEFAULT_VC_CONTEXT, DEFAULT_VC_TYPE } from './verifiable-credential.js';
...
export class StatusListCredential {
    ...
    static create(options) {
        ...
        const indexesOfCredentialsToRevoke = this.validateStatusListEntryIndexesAreAllUnique(statusPurpose, credentialsToDisable);
     ...
     * ```
     */
    static validateCredentialInStatusList(credentialToValidate, statusListCredential) {
        const statusListEntryValue = credentialToValidate.vcDataModel.credentialStatus;
        const credentialSubject = statusListCredential.vcDataModel.credentialSubject;
        const statusListCredStatusPurpose = credentialSubject['statusPurpose'];
        const encodedListCompressedBitString = credentialSubject['encodedList'];
     ...
     */
    static validateStatusListEntryIndexesAreAllUnique(statusPurpose, credentials) {
        const uniqueIndexes = new Set();
        for (const vc of credentials) {
            if (!vc.vcDataModel.credentialStatus) {
                throw new Error('no credential status found in credential');
            }
            const statusList2021Entry = vc.vcDataModel.credentialStatus;
     ...
     */
    static generateBitString(indexOfBitsToTurnOn) {
     ...
     */
    static getBit(compressedBitstring, bitIndex) {
