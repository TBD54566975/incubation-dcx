...
function wildcardToRegex(stringWithWildcard) {
  const regexString = stringWithWildcard.split(/\*+/).map((s) => escapeStringRegexp(s)).join(".*");
...
async function runYargs() {
  const { argv } = yargs(hideBin(process.argv)).config(
    "config",
    (configPath) => (
...
function yarnSupportsClassicAudit(yarnVersion) {
  return semver2.satisfies(yarnVersion, `^${MINIMUM_YARN_CLASSIC_VERSION}`);
}
function yarnSupportsBerryAudit(yarnVersion) {
  return semver2.gte(yarnVersion, MINIMUM_YARN_BERRY_VERSION);
}
...
function yarnAuditSupportsRegistry(yarnVersion) {
  return semver2.gte(yarnVersion, MINIMUM_YARN_AUDIT_REGISTRY_VERSION);
}
...
function getYarnVersion(yarnExec = "yarn", cwd) {
  ...
  if (version)
    return version;
  version = execSync2(`${yarnExec} -v`, { cwd }).toString().replace("\n", "");
...
//# sourceMappingURL=chunk-FA3SOWIW.js.map